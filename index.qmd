...
quant %>%
  ggplot +
  for (col in colnames(quant)){geom_histogram(x = col, binwidth = +
  facet_wrap(colnames(quant))
```
...
```{R}
map2(x = num_rings, y = col, color = sex)%>%
  cowplot::plot_grid()
```
...
predict_model = predict(model, newdata = nd_frame)
print(predict_model)
plot(df$diameter, df$height)
plot(nd_frame, predict_model)
df %>%
  ggplot +
  geom_point(aes(x = )
