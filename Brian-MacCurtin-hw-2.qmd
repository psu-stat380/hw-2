...
```{r}
df2 %>%
  select(meanlength, meandiameter, meanheight, meanwhole_weight, meanshucked_weight, meanviscera_weight, meanshell_weight, meanrings) %>%
  gather() %>%
  ggplot(aes(x=value)) +
  facet_wrap(~key, scales = "free") +
  geom_bar()
...
```{r}
variables <- c("length", "diameter", "height", "whole_weight", "shucked_weight", "viscera_weight", "shell_weight", "rings")
